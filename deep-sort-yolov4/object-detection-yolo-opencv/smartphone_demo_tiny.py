...
def setup_door(H1, W1, H2, W2, R):
    # bước 1 tìm trung điểm của A(W1, H1) B(W2, H2)
    I1 = (W1 + W2) / 2
    I2 = (H1 + H2) / 2
...
def get_box_dimensions(outputs, height, width, defaultConfidence):
    boxes = []
    confs = []
    class_ids = []
    ...
    for i in np.arange (0, outputs.shape[2]):
        ...
        if confidence > defaultConfidence:
            ...
            # for the object
            box = outputs[0, 0, i, 3:7] * np.array ([width, height, width, height])
...
def detect_image(img, height, width, defaultConfidences):
    # define model with weight and configuration (here choose yolov3 320x320
    net = cv2.dnn.readNetFromCaffe ("mobilenet_ssd/MobileNetSSD_deploy.prototxt",
                                    "mobilenet_ssd/MobileNetSSD_deploy.caffemodel")
    # turn on if have cuda
    # net.setPreferableBackend(cv2.dnn.DNN_BACKEND_CUDA)
    # net.setPreferableTarget (cv2.dnn.DNN_TARGET_CUDA)

    # get coco classes name
    with open("coco.names", "r") as f:
        classes = [line.strip () for line in f.readlines ()]

    # output_layers = ['yolo_82', 'yolo_94', 'yolo_106']
    blob = cv2.dnn.blobFromImage(img, scalefactor=0.00392, size=(width, height), mean=(0, 0, 0), swapRB=True,
                                crop=False)
    net.setInput(blob)
    outputs = net.forward()
    boxes, confs, class_ids = get_box_dimensions(outputs, height, width, defaultConfidences)
    return boxes, confs, class_ids
...
def main():
    ...
    while True:
        cam_id, frame = image_hub.recv_image()  # change to receive from camera
        ...
        if totalFrames % defaultSkipFrames == 0:
            boxes, confidence, classes = detect_image(frame, H, W, defaultConfidences)  # average time: 1.2s
